  ...
  return (
    <main style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) 360px', gap: 16 }}>
      <section style={PANEL_STYLE} aria-label={t('HTTP/2 streams', 'HTTP/2 のストリーム')}>
        ...
        >
          {loss
            ? t('Disable TCP Loss', 'TCP のパケットロスを止める')
            : t('Enable TCP Loss', 'TCP のパケットロスを起こす')}
        </button>
        <div style={{ display: 'grid', gap: 8, marginTop: 12 }}>
          {run.streams.map((stream) => (
            <div key={stream.id} data-testid={`h2-stream-${stream.id}`}>
              {t('Stream', 'ストリーム')} {stream.id} {stream.path}:{' '}
              {stream.status === 'stalled' ? t('stalled', '停止中') : t('complete', '完了')}
            </div>
          ))}
        </div>
      </section>
      <aside style={PANEL_STYLE} aria-label={t('HTTP/2 frame order', 'HTTP/2 のフレームの順序')}>
