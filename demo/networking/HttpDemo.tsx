...
function resolveServerResponse(
  ...
  url: string,
  requestBody?: string,
): { statusCode: number; reasonPhrase: string; body: string; headers: Record<string, string> } {
  if (method === 'GET' && url === '/') {
    return {
      statusCode: 200,
      reasonPhrase: 'OK',
...
function HttpDemoInner() {
  ...
  const sendHttp = async (method: string, url: string, body?: string) => {
    ...
    try {
      ...
      const session = sessions.find((s) => s.sessionId === sessionId) ?? { httpMeta: undefined };
      if (!session.httpMeta) {
      ...
      const responsePayload: HttpMessage = {
        ...
        headers: { ...response.headers, Server: 'netlab/0.1', Connection: 'close' },
        body: response.body,
      };
    ...
    <div style={{ display: 'flex', height: '100%' }}>
      ...
      <ResizableSidebar defaultWidth={340} minWidth={280} maxWidth={600}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, height: '100%' }}>
          ...
          <div tabIndex={0} style={{ flex: 1, minHeight: 0, overflow: 'auto' }}>
            <SessionDetail />
          </div>
        </div>
      </ResizableSidebar>
