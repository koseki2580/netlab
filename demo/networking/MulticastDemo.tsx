...
const BTN_PRIMARY: CSSProperties = { ...BTN_BASE, background: '#0f766e', color: '#f0fdfa' };
const BTN_JOIN: CSSProperties = { ...BTN_BASE, background: '#065f46', color: '#d1fae5' };
const BTN_LEAVE: CSSProperties = { ...BTN_BASE, background: '#7f1d1d', color: '#fee2e2' };

/* ── Packet builder ─────────────────────────────── */

function buildMulticastPacket(srcNodeId: string, dstNodeId: string, srcIp: string): InFlightPacket {
  return {
    id: `mcast-${Date.now()}`,
    srcNodeId,
    dstNodeId,
...
function MulticastDemoInner() {
  ...
  const toggle = (receiver: ReceiverInfo) => {
    ...
    if (isJoined) {
      engine.removeMulticastMembership('switch-1', receiver.vlanId, MULTICAST_MAC, receiver.portId);
      ...
      engine.addMulticastMembership('switch-1', receiver.vlanId, MULTICAST_MAC, receiver.portId);
