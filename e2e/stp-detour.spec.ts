 ...
 *
 * The lesson's own brief reads: "Press B → C: the blocked segment is not used;
 * traffic detours through Switch A." It detoured
...
test('the spanning-tree lesson delivers B to C around the blocked segment', async ({
  ...
  // The lesson opens idle; the learner sends the ping the brief describes.
  await page.getByTestId(SEL.stp.ping('bc')).click();
