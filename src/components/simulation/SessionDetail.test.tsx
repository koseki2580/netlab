...
describe('SessionDetail', () => {
  ...
  it('formats elapsed event times relative to session start', () => {
    sessionMock.selectedSession = makeSession({
      events: [{ phase: 'request:initiated', timestamp: 105, seq: 0, nodeId: 'client-1' }],
    });
  ...
  it('resolves node labels and falls back to raw ids when missing', () => {
    sessionMock.selectedSession = makeSession({
      events: [{ phase: 'request:initiated', timestamp: 105, seq: 0, nodeId: 'missing-node' }],
    });
  ...
  it('renders session events in order', () => {
    ...
    expect(html.indexOf('request · initiated')).toBeLessThan(html.indexOf('request · delivered'));
  });
  ...
  it('renders request trace path with hops', () => {
    sessionMock.selectedSession = makeSession({
      requestTrace: {
        ...
        status: 'delivered',
        hops: [
          makeHop({ nodeLabel: 'Client' }),
          makeHop({ step: 1, nodeId: 'server-1', nodeLabel: 'Server', event: 'deliver' }),
        ],
      },
    });
...
describe('SessionDetail — HTTP panes', () => {
  it('renders the request pane with headers and body', () => {
    sessionMock.selectedSession = makeSession({
      httpMeta: {
        method: 'POST',
        path: '/echo',
        statusCode: 200,
        requestHeaders: { 'Content-Type': 'text/plain', Host: 'example.com' },
        requestBody: 'Hello',
        responseHeaders: {},
        responseBody: 'Echo: Hello',
      },
    });
    ...
    expect(html).toContain('POST /echo HTTP/1.1');
    expect(html).toContain('Content-Type');
    expect(html).toContain('text/plain');
    expect(html).toContain('Hello');
  });

  it('renders the response pane with headers and body', () => {
    sessionMock.selectedSession = makeSession({
      httpMeta: {
        method: 'GET',
        path: '/',
        statusCode: 200,
        requestHeaders: {},
        responseHeaders: { Server: 'netlab/0.1' },
    ...
    expect(html).toContain('HTTP RESPONSE');
    expect(html).toContain('HTTP/1.1 200');
    expect(html).toContain('Server');
    expect(html).toContain('netlab/0.1');
    expect(html).toContain('OK body');
  });

  it('collapses bodies > 500 chars by default', () => {
    ...
    // The collapse button should appear with char count
    expect(html).toContain('Show all (600 chars)');
  });

  it('perf: is wrapped in React.memo', () => {
    expect((SessionDetail as unknown as { $$typeof: symbol }).$$typeof).toBe(
