...
function make3PortTopology(): { topology: NetworkTopology; ports: SwitchPort[] } {
  ...
  const topology: NetworkTopology = {
    ...
    edges: [
      { id: 'e1', source: 'sw-1', sourceHandle: 'port-1', target: 'host-a', targetHandle: 'eth0' },
      { id: 'e2', source: 'sw-1', sourceHandle: 'port-2', target: 'host-b', targetHandle: 'eth0' },
      { id: 'e3', source: 'sw-1', sourceHandle: 'port-3', target: 'host-c', targetHandle: 'eth0' },
    ],
    areas: [],
    routeTables: new Map(),
  };
...
function makeIgmpReportPacket(srcMac: string, srcIp: string, groupAddress: string): InFlightPacket {
...
function makeIgmpLeavePacket(srcMac: string, srcIp: string, groupAddress: string): InFlightPacket {
...
describe('SwitchForwarder — multicast snooping', () => {
  ...
  it('forgets the membership for (vlan1, group) on Leave from the same port', async () => {
    // First join
    const reportPkt = makeIgmpReportPacket('aa:00:00:00:00:01', '10.0.0.10', GROUP);
    await fw.receive(reportPkt, 'port-1', { neighbors: [] });
    // Then leave
    const leavePkt = makeIgmpLeavePacket('aa:00:00:00:00:01', '10.0.0.10', GROUP);
  ...
  it('keeps other ports in the group after one port leaves', async () => {
    ...
    const report1 = makeIgmpReportPacket('aa:00:00:00:00:01', '10.0.0.10', GROUP);
    await fw.receive(report1, 'port-1', { neighbors: [] });
    const report2 = makeIgmpReportPacket('aa:00:00:00:00:02', '10.0.0.20', GROUP);
