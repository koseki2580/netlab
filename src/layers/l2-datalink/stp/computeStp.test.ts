  ...
  describe('topology without loops', () => {
    it('no switches → root is null, ports map is empty', () => {
      const result = computeStp(
        makeTopology(
          [makeLeaf('client-1', 'client'), makeLeaf('server-1', 'server')],
    ...
    it('single switch → every port is DESIGNATED / FORWARDING (no loop possible)', () => {
      ...
      expect(
        [...result.ports.values()].every(
          (port) => port.role === 'DESIGNATED' && port.state === 'FORWARDING',
        ),
      ).toBe(true);
    });
      ...
      const result = computeStp(
        makeTopology(
          [
            ...
            makeSwitch('switch-c', [{ id: 'cb', name: 'fa0/1', macAddress: '02:00:00:0c:00:01' }]),
          ],
          [
            makeEdge('e-ab', 'switch-a', 'switch-b', 'ab', 'ba'),
            makeEdge('e-bc', 'switch-b', 'switch-c', 'bc', 'cb'),
