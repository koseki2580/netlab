...
import {
  buildUdpPacket,
  generateEphemeralPort,
  type UdpPacketOptions,
} from "./udpPacketBuilder";

function makeOptions(
  overrides: Partial<UdpPacketOptions> = {},
): UdpPacketOptions {
  return {
    srcNodeId: "client-1",
    dstNodeId: "server-1",
...
describe("buildUdpPacket", () => {
  ...
  it("throws when srcPort < 0", () => {
    expect(() => buildUdpPacket(makeOptions({ srcPort: -1 }))).toThrow(
      RangeError,
    );
  });

  it("throws when dstPort > 65535", () => {
    expect(() => buildUdpPacket(makeOptions({ dstPort: 65536 }))).toThrow(
      RangeError,
    );
  });
