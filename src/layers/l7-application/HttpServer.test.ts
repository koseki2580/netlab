...
import { buildHttpRequest, buildHttpResponse, serializeHttp } from './httpPacketBuilder';

function makeGetRequest(url: string, requestId = 'r1'): HttpMessage {
  return buildHttpRequest({ method: 'GET', url, host: 'test.local', requestId });
}
...
describe('HttpServer', () => {
  ...
  it('responds via the matching handler for an exact path', async () => {
    ...
    server.route('GET', '/hello', () =>
      buildHttpResponse({ statusCode: 200, reasonPhrase: 'OK', requestId: 'r1', body: 'world' }),
    );
    server.listen();
  ...
  it('buffers a request split across multiple chunks before dispatching', async () => {
    const server = new HttpServer({ nodeId: 'srv' });
    server.route('GET', '/', () =>
      buildHttpResponse({ statusCode: 200, reasonPhrase: 'OK', requestId: 'r1' }),
    );
    server.listen();
