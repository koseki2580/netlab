...
describe('TraceRecorder', () => {
  ...
  it('merges secondary precompute results with step offsets and merged ARP tables', () => {
    const recorder = new TraceRecorder();
    const primaryPacket = makePacket('trace-merge-primary', 'client-1', 'server-1', '10.0.0.10', '203.0.113.10');
    const secondaryPacket = makePacket('trace-merge-secondary', 'server-1', 'client-1', '203.0.113.10', '10.0.0.10');
  ...
  it('stores and returns snapshots by packet id', () => {
    const recorder = new TraceRecorder();
    const packet = makePacket('trace-snapshots', 'client-1', 'server-1', '10.0.0.10', '203.0.113.10');
