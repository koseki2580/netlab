...
describe("serializePacket", () => {
  ...
  it("serializes ICMP payloads without treating them as UDP", () => {
    ...
    expect(
      fields.find((field) => field.name === "Protocol")?.displayValue,
    ).toBe("1 (ICMP)");
    expect(fields.find((field) => field.name === "Type")?.displayValue).toBe(
      "8",
    );
    expect(
      fields.find((field) => field.name === "Sequence Number")?.displayValue,
    ).toBe("1");
  });
  ...
  it("renders the materialized IPv4 checksum in the field table", () => {
    ...
    expect(
      fields.find((field) => field.name === "Header Checksum")?.displayValue,
    ).toBe("0xABCD");
    expect(
      fields.find((field) => field.name === "Identification")?.displayValue,
    ).toBe("0x1234");
  });
});
